...
function resolveDirectory (path) {
    return FS.statSync(path).isFile() ? Path.dirname(path) : path;
}
...
function sakeRequire (name) {
    ...
    try {
        ...
        paths = [__dirname, process.cwd(), resolveDirectory(CURRENT_PATH)];
        i = paths.length;
        while (i-- && !modpath) {
...
Proteus.merge(sake, Object.defineProperties({
    ...
    run: function (filepath) {
        ...
        if (typeof filepath === "function") {
            ...
            filepath = Path.resolve(resolveDirectory(CURRENT_PATH), filepath);
    ...
    __dirname: {
        get: function () {
            return resolveDirectory(CURRENT_PATH);
        },
        enumerable: true
    },
