...
var Proteus         = require("proteus"),
    ...
    PRECOMPILE_FUN_REGION_START_RE         = /^(.*)\ssakefile:\s*{/,
    PRECOMPILE_FUN_REGION_END_RE           = /\ssakefile:\s*}/,
...
//---------------------------------------------------------------------------
// Get source of bootstrap function for precompiling Sakefile.
//
...
// is returned.
//---------------------------------------------------------------------------
function extractPrecompileFunctionSource (sakefileSource) {
    var codeLineNum = 0,
        functionRegionLinesRead = 0,
        functionSource = "",
        newlinePos = 0,
        pos = 0,
        codeLine,
    ...
    while (pos < sakefileSource.length) {
        // Find next code line.
        newlinePos = sakefileSource.indexOf("\n", pos);
        if (newlinePos === -1) {
            newlinePos = sakefileSource.length
        }
        codeLine = sakefileSource.substr(pos, newlinePos - pos);
        pos = newlinePos + 1;

        if (!functionRegionLinesRead) {
            // Not in function region yet.
            // Test read code line for being marker of function region start.
            if (marker = PRECOMPILE_FUN_REGION_START_RE.exec(codeLine)) {
                functionRegionLinesRead = 1;
                markerLinePrefix = marker[1];
            }
            else {
                // Count read code lines and check for exceeding a limit of
                // lines to look in for marker of function region start.
                codeLineNum++;
                if (codeLineNum >= PRECOMPILE_FUN_REGION_SEARCH_LINES_MAX) {
                    return "";
                }
            }
            continue;
        ...
        if (PRECOMPILE_FUN_REGION_END_RE.test(codeLine)) {
            return functionSource;
        }

        functionSource += codeLine.replace(markerLinePrefix, "") + "\n";

        functionRegionLinesRead++;
        if (functionRegionLinesRead >= PRECOMPILE_FUN_REGION_LINES_MAX) {
            return "";
        }
    }
}
...
Proteus.merge(sake, Object.defineProperties({
    ...
    run: function (filepath) {
        var code, ret, precompileFunSource;
        ...
        if (typeof filepath === "function") {
            ...
            if (Path.extname(filepath) === "") {
                precompileFunSource = extractPrecompileFunctionSource(code);
                if (precompileFunSource.length > 0) {
                    try {
                        code = eval("(" +
                          precompileFunSource.replace(/;\s*$/, "") +
                        ")")(code);
                    }
                    catch (e) {
                        log.error("Precompile function failed. " + e + "\n" +
            ...
            else if (Path.extname(filepath).toLowerCase() === ".coffee") {
                try {
...
Object.defineProperty(module, "exports", {
    get: function () {
        ...
        // load sakelib files
        opts.sakelib.map(function (dirpath) {
            return Path.resolve(dirpath);
        }).filter(function (dirpath) {
            log.debug('sakelib: ' + dirpath);
