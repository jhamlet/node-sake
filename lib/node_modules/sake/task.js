...
function runActions (callback) {
    ...
    if (actions.length) {
        async.series(actions.map(function (action, idx) {
            return function (cb) {
                self.on("actionComplete", function () {
                    self.removeListener("actionComplete", arguments.callee);
                    cb();
                });
                // console.log(action.toString());
                action.apply(self, [self].concat(self.arguments));
            };
        }), actionsCallback);
    }
    else {
...
function runPrerequisites (callback) {
    var self = this,
        prereqs = this.prerequisites
    ;
    
    function prereqsCallback () {
        runActions.call(self, callback);
    }
    
    if (prereqs.length) {
        async.series(prereqs.map(function (preqName) {
            return function (cb) {
                var task = Task.get(preqName);
                task.on("complete", function () {
                    task.removeListener("complete", arguments.callee);
                    cb();
                });
                task.invoke();
            };
...
module.exports = Task = Proteus.Class.derive({
    ...
    init: function (name, prereqs, action) {
        ...
        Task.tasks[this.name + ":" + this.namespace] = this;
    ...
    invoke: function () {
        if (!this.alreadyRun && this.isNeeded) {
            ...
            // console.log("alreadyRun: " + this.alreadyRun);
            // console.log("isNeeded: " + this.isNeeded);
    ...
    execute: function () {
        ...
        if (!this.running) {
            ...
            runPrerequisites.call(this, function () {
                self.running = false;
    ...
    complete: Proteus.aliasMethod("done"),
