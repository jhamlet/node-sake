...
module.exports = {
    ...
    listTasksAndPrerequisites: (function () {
        ...
        function formatTask (task, pad) {
            return (pad ? pad : "") + task.name + " (" + task.type + ")";
            // return (pad ? pad : "") + task.type + ": " + task.name;
        }
        
        return function () {
            console.log("Tasks and Prerequisites: ");
            
            Task.getAll().forEach(function (task) {
                var preqs       = task.prerequisites,
                    pad         = "  "
                ;

                console.log(formatTask(task, pad));
                preqs.forEach(function (name) {
                    console.log(formatTask(Task.lookup(name), pad + pad));
                });
            });
        };
    }()),
