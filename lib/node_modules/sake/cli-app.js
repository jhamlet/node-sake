...
var Proteus = require("proteus"),
    ...
    SAKEFILE_GLOB_PATTERN = "[Ss]akefile{.js,.coffee,}"
;
...
module.exports = {
    ...
    run: function () {
        ...
        Task.invoke.apply(Task, [opts.taskname].concat(args));
    },
    
    processRunOptions: function () {
        var opts    = this.options,
            pattern = opts.taskname,
            exit    = false;
        
        pattern = pattern === "default" ? false : new RegExp(pattern);
        
        if (opts.tasks) {
            this.listTasks(pattern);
            exit = true;
        }
        else if (opts.prereqs) {
            this.listPrerequisites(pattern);
            exit = true;
        }
    ...
    haveSakefile: function (here) {
        var pattern = Path.join(here, SAKEFILE_GLOB_PATTERN),
