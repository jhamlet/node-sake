...
ActionRunner = Runner.derive({
    ...
    makeActionCallback: function (idx, action, fn, callback, abort) {
        var task        = this.task,
            isDryRun    = options.dryrun,
            dryRunSfx   = isDryRun ? " (dry-run)" : ""
        ;
        
        if (task !== this.task || action !== fn.action) {
            log.debug(task.fqn + " action callback called out of context.");
            return;
        }
        
        task.removeListener("actionDone", arguments.callee);
        task.removeListener("abort", arguments.callee);

        log.debug(
            task.fqn + " action " + idx +
            (abort && abort !== task ? " aborted." : " complete" + dryRunSfx + ".")
        );
    ...
    makeAsyncCallback: function (action, idx) {
        ...
        asyncFn = function (asyncCb) {
            ...
            cb = actionCb.bind(self, idx, action, asyncFn, asyncCb);
            
            task.on("actionDone", cb);
            task.on("abort", cb);
